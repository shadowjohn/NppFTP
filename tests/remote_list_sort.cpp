...
int main()
{
	RemoteListSortItem dirA = item((void*)1, true, 4096, 20, TEXT("\u76EE\u9304A"), "/\u76EE\u9304A", "drwxr-xr-x");
	RemoteListSortItem dirB = item((void*)2, true, -1, 10, TEXT("\u76EE\u9304B"), "/\u76EE\u9304B", "drwx------");
	...
	assert(remote_list_sort_less(dirB, dirA, RemoteSortSize, true));
	assert(remote_list_sort_less(dirA, dirB, RemoteSortSize, false));
	assert(remote_list_sort_less(dirA, smallFile, RemoteSortSize, true));
	assert(remote_list_sort_less(dirA, smallFile, RemoteSortSize, false));
	assert(!remote_list_sort_less(smallFile, dirA, RemoteSortSize, true));
	assert(!remote_list_sort_less(smallFile, dirA, RemoteSortSize, false));
